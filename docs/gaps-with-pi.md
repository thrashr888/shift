...
| Production hardening | The evaluator's authority surface is a committed allowlist with an audit suite, the parsers have a seeded fuzz suite, `.env` and `op` secrets are redacted everywhere they are written, `trace-content` bounds what traces keep, and the suite runs on Ubuntu in CI. Still open: resource limits for jobs and symlink/race analysis. |
...
| Built-in subagents and plan mode | Pi deliberately omits these and expects packages to add them. This harness has `spawn`: parallel generation-pinned children as session folders under their parent, with linked traces and a durable narrower authority ceiling, and a plan mode that permits only read-only tools. Children still share the working tree. |
