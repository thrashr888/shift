...
| [skills-and-jobs-rfc.md](skills-and-jobs-rfc.md) | RFC for skills, background jobs, concurrent read-only tools, and the paginated sidebar; implemented except `source` rows. |
...
- **Sidebar sources.** `source` rows so the built-in Session and Log tabs
  become default panes a project can recompose
