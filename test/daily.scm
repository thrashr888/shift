...
(test-eq "autopilot allows behavior changes" 'allow (tool-decision 'autopilot "live_eval" (json-object)))
(test-eq "autopilot permits scoped edits" 'allow (tool-decision 'autopilot "edit" (json-object)))
...
(test-eq "autopilot permits patches" 'allow (tool-decision 'autopilot "apply_patch" (json-object)))
(test-eq "autopilot runs without asking" 'allow (tool-decision 'autopilot "run" (json-object)))
...
(test-eq "autopilot allows MCP tools" 'allow (tool-decision 'autopilot "fake__echo" (json-object)))
...
(test-eq "autopilot allows every run" 'allow (tool-decision 'autopilot "run" cargo-test '()))
