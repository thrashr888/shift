...
(use-modules (live-agent redact))
(define (last-span tracer)
  (json-read (car (last-pair (string-split (string-trim-right (call-with-input-file (tracer-path tracer) get-string-all)) #\newline)))))
(define private-tracer (make-tracer (string-append test-root "/other") #f "private-id" "private"))
(register-secret! "TOKEN" "hunter2hunter2")
(let ((span (trace-start! private-tracer "tool.run" "TOOL" '((input.value . "echo hunter2hunter2")))))
  (trace-end! span "OK" `((output.value . ,(make-string 300 #\x)))))
(let ((attributes (json-object-ref (last-span private-tracer) "attributes")))
  (test-equal "secrets are redacted in trace attributes" "echo [redacted TOKEN]" (json-object-ref attributes "input.value"))
  (test-equal "full mode keeps long content" 300 (string-length (json-object-ref attributes "output.value"))))
(parameterize ((trace-content 'bounded))
  (let ((span (trace-start! private-tracer "tool.run" "TOOL" '((input.value . "x")))))
    (trace-end! span "OK" `((output.value . ,(make-string 300 #\y)))))
  (test-equal "bounded mode clips content to 200 characters" 201
    (string-length (json-object-ref (json-object-ref (last-span private-tracer) "attributes") "output.value"))))
(parameterize ((trace-content 'off))
  (let ((span (trace-start! private-tracer "tool.run" "TOOL" '((input.value . "x") (tool.name . "run")))))
    (trace-end! span "ERROR" '((output.value . "y") (error.message . "boom"))))
  (let ((attributes (json-object-ref (last-span private-tracer) "attributes")))
    (test-assert "off mode drops content but keeps names and errors"
      (and (not (json-object-ref attributes "output.value" #f)) (not (json-object-ref attributes "input.value" #f))
           (equal? "run" (json-object-ref attributes "tool.name")) (equal? "boom" (json-object-ref attributes "error.message"))))))
(forget-secrets!)
(trace-close! private-tracer)
