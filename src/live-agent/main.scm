...
(define (host-command-allowed? command)
  (let ((parts (string-tokenize command)))
    (or (member command '("/mode manual" "/mode plan" "/mode accept" "/mode auto"))
      ...
      (ui-host-handler!
        (lambda (command)
          (unless (host-command-allowed? command)
            (error "only /mode manual|plan|accept|auto, /model list, or /model PROVIDER/MODEL is supported"))
          (unless (try-mutex lock) (error "Session busy; finish the turn or pending approval before changing mode or model"))
          (dynamic-wind
            (lambda () #t)
            (lambda ()
              (if (string=? command "/model list")
