...
(define max-job-timeout 3600)
(define max-jobs 4)
(define job-tail-bytes (* 16 1024))
...
(define (parse-run-arguments arguments root)
  (let* ((argv-value (json-object-ref arguments "argv" #f))
         ...
         (ceiling (if background? max-job-timeout max-timeout)))
    ...
    (unless (and (integer? timeout) (>= timeout 1) (<= timeout ceiling))
      (error (format #f "timeout_seconds must be an integer from 1 through ~a" ceiling)))
...
(define (execute-run arguments root ledger turn context)
  (if (json-object-ref arguments "background" #f)
...
;; --- background jobs ------------------------------------------------------
;; A job is a run that returns at once. The process lives under this session:
;; a reader thread streams its output to the ledger's log file and a bounded
;; tail, the deadline and cancellation reuse terminate!, and completion lands
;; in the ledger as one run record plus a notice the turn loop hands to the
;; model. Process exit kills every job; nothing is daemonized.
(define jobs-lock (make-mutex))
(define jobs '())                     ; newest first
(define job-notices '())              ; strings, oldest first
...
(define (take-job-notices)
  (with-mutex jobs-lock (let ((taken job-notices)) (set! job-notices '()) taken)))
(define (job-elapsed job)
  (if (job-running? job) (elapsed-ms (job-started job)) (job-duration job)))
(define (last-bytes bytes limit)
  (if (> (bytevector-length bytes) limit)
      (let ((out (make-bytevector limit)))
        (bytevector-copy! bytes (- (bytevector-length bytes) limit) out 0 limit)
        out)
      bytes))
(define (append-bytes a b)
  (let ((out (make-bytevector (+ (bytevector-length a) (bytevector-length b)))))
    ...
    out))
(define (tail-text bytes)
  (string-trim-right (bytevector->string (last-bytes bytes job-tail-bytes) "UTF-8" 'substitute)))
(define (job-event job kind)
  (let ((tag (job-tag job)))
    (json-object (cons "event" kind) (cons "id" (job-id job))
                 (cons "argv" (apply json-array (job-argv job)))
                 (cons "status" (symbol->string (job-status job)))
                 (cons "code" (or (job-code job) json-null))
                 (cons "ok" (and (eq? (job-status job) 'exit) (eqv? (job-code job) 0)))
                 (cons "elapsed_ms" (job-elapsed job))
...
(define (job-status-text job)
  (case (job-status job)
    ((running) "running")
    ((exit) (format #f "exit ~a" (job-code job)))
    ((signal) (format #f "killed by signal ~a" (job-code job)))
    ((timeout) (format #f "timeout after ~as (killed)" (job-code job)))
    ((cancelled) "cancelled")
    (else (symbol->string (job-status job)))))
(define (job-summary job)
  (format #f "job ~a ~a · ~a · ~as · log ~a"
          (job-id job) (if (job-running? job) "running" "finished") (job-status-text job)
          (/ (round (/ (job-elapsed job) 100.0)) 10.0) (job-log job)))
(define (finish-job! job ledger status code log-path)
  (set-job-status! job status) (set-job-code! job code)
  (set-job-duration! job (elapsed-ms (job-started job)))
  (ledger-record-run! ledger (job-turn job)
    (json-object
     (cons "invocation" (json-object (cons "mode" "local")
                                     (cons "input" (json-object (cons "command" (apply json-array (job-argv job)))
                                                                (cons "workdir" (job-workdir job))))))
     (cons "outcome" (json-object (cons "exit_code" (if (eq? status 'exit) code -1))
                                  (cons "success" (and (eq? status 'exit) (eqv? code 0)))
                                  (cons "output_sha256" (sha256-file log-path))
     ...
     (cons "job" (job-id job))))
  (notify! job)
  (catch #t (lambda () (observer (job-event job "finished"))) (lambda _ #f)))
(define (job-log-path ledger)
  (let* ((directory (string-append (ledger-directory ledger) "/runs"))
         (name (format #f "job-~a.log" job-counter)))
    (unless (file-exists? (ledger-directory ledger)) (mkdir (ledger-directory ledger)))
    (unless (file-exists? directory) (mkdir directory))
...
(define (cancel-job! id)
  (let ((job (job-find id)))
    ...
    (if (job-running? job)
        (begin (set-job-status! job 'cancelled)
               (catch #t (lambda () (kill (job-pid job) SIGTERM)) (lambda _ #f))
               (format #f "job ~a cancelled" id))
        (format #f "job ~a already finished: ~a" id (job-status-text job)))))
(define (stop-jobs! reason)
  (for-each (lambda (job)
...
(define (execute-job arguments)
  (let ((action (json-object-ref arguments "action" "list"))
        ...
        (timeout (json-object-ref arguments "timeout_seconds" 60)))
    (cond
     ((string=? action "list")
      (let ((all (job-list)))
        (make-tool-result #t (if (null? all) "No jobs in this session."
                                 (string-join (map job-summary all) "\n")))))
     ((not (string? id)) (error "id is required for wait, output and cancel"))
     ((string=? action "output")
      (let ((job (job-find id)))
        (unless job (error "no job with that id" id))
        (make-tool-result (not (memq (job-status job) '(signal timeout cancelled)))
                          (job-output-text job))))
     ((string=? action "wait")
      (unless (and (integer? timeout) (<= 1 timeout 600)) (error "timeout_seconds must be 1 through 600"))
      (let ((job (job-find id)) (deadline (+ (get-internal-real-time) (* timeout internal-time-units-per-second))))
        (unless job (error "no job with that id" id))
        (let loop ()
          (cond ((not (job-running? job))
                 (take-job-notices) ; the caller has the outcome now
                 (make-tool-result (and (eq? (job-status job) 'exit) (eqv? (job-code job) 0)) (job-output-text job)))
                ((> (get-internal-real-time) deadline)
                 (make-tool-result #t (string-append (job-summary job) "\nStill running after " (number->string timeout) "s; wait again or continue.")))
                (else (usleep 200000) (loop))))))
     ((string=? action "cancel") (make-tool-result #t (cancel-job! id)))
     (else (error "action must be list, wait, output or cancel")))))
  ...
  (catch #t
    (lambda ()
      (cond
       ...
       ((string=? name "job") (execute-job arguments))
    ...
    (function-tool
     ...
     (json-object
      ...
      (cons "timeout_seconds"
            (json-object (cons "type" "integer") (cons "minimum" 1) (cons "maximum" max-job-timeout)
                         (cons "description" "Kill the command after this many seconds; defaults to 120, at most 600 in the foreground and 3600 in the background")))
      (cons "background"
            (json-object (cons "type" "boolean")
                         (cons "description" "Return at once with a job id and keep the command running; a notice arrives when it finishes. Use for long test suites and builds."))))
     '("argv")))
   ((string=? name "job")
    (function-tool
     "job"
     "Background jobs started by run: list them, wait for one, read its output so far, or cancel it."
     (json-object
      (cons "action" (string-parameter "list, wait, output or cancel"))
      (cons "id" (string-parameter "Job id from run, for wait, output and cancel"))
      (cons "timeout_seconds" (json-object (cons "type" "integer") (cons "minimum" 1) (cons "maximum" 600)
                                            (cons "description" "How long wait blocks; defaults to 60"))))
     '("action")))
